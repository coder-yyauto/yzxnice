        ...
        with ui.column().classes("w-full max-w-4xl p-6"):
            ...
            if is_super:
                with ui.row().classes("gap-2 mb-4"):
                    school_name_input = ui.input(label="学校名称", placeholder="如：实验一小").classes("w-48").props(
                        "outlined dense"
                    )
                    school_code_input = ui.input(
                        label="学校代码", placeholder="如：xx001"
                            ...
                            with ui.row().classes("gap-2"):
                                for cls in classes:
                                    count = db.query(User).filter(
                                        User.default_org_id == cls.id, User.user_type == "student"
                                    ).count()
...
def _show_teacher_dialog(school_id, container, admin_schools=None):
    ...
    with ui.dialog() as dialog, ui.card().classes("w-96"):
        ...
        username_input = ui.input(label="用户名", placeholder="如：Txx001001").classes("w-full").props(
            "outlined dense"
        )
...
def _reset_passwords(school_id, admin_schools=None):
    ...
    with get_db() as db:
        ...
        teachers = db.query(User).filter(
            User.default_org_id == school_id, User.user_type == "teacher"
        ).all()
        for t in teachers:
                                ...
                                for cls in classes:
                                    ...
                                    if students:
                                        with ui.expansion(f"{cls.name} ({len(students)}人)").classes("pl-6"):
                                            ...
                                            if len(students) > 10:
                                                ui.label(f"...等共 {len(students)} 人").classes(
        ...
        with get_db() as db:
            ...
            async def assign_role():
                ...
                if not uid or not role or not scope:
                    ...
                    return
                with get_db() as db2:
                    assigner = db2.query(User).filter(
                        User.id == app.storage.user.get("user_id")
                    ).first()
                    if not assigner:
            ...
            if not is_super:
                ui.label("提示：校级管理员只能由超级管理员赋权").classes(
            ...
            roles = (
                ...
                .filter(User.default_org_id == school_id)
                .all()
            )
            if roles:
                rows = []
                ...
                with ui.column().classes("w-full gap-2"):
                    for row in rows:
                        with ui.row().classes(
                            "w-full items-center bg-white rounded p-3 shadow-sm"
                        ):
