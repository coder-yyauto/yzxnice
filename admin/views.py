...
async def admin_index():
    ...
    render_admin_navbar()
    with (
        ui.column().classes("w-full p-6 min-h-screen bg-gray-50 items-center"),
        ui.column().classes("w-full max-w-4xl"),
    ):
...
async def admin_org():
    ...
    with (
        ...
        if is_super:
            ...
            ui.button("创建学校", icon="add", on_click=on_create_click).props("color=primary")
        else:
...
async def _create_school_from_inputs(name: str, code: str) -> None:
    name = name.strip()
    code = code.strip()
...
def _user_row(user, type_label):
    return {
        ...
        "状态": "正常" if user.is_active else "停用",
        "类型": type_label,
    }
        ...
        for cls in classes:
            ...
            if students:
                with ui.expansion(f"{cls.name} ({len(students)}人)").classes("pl-6"):
                    _render_user_table([_user_row(s, "学生") for s in students[:10]])
                    if len(students) > 10:
            ...
            with ui.expansion(f"{school.name}", icon="school").classes("w-full mb-2"):
                ...
                if teachers:
                    with ui.expansion(f"教师 ({len(teachers)}人)").classes("pl-4"):
                        _render_user_table([_user_row(t, "教师") for t in teachers])

                for grade in get_grades(db, school.id):
                    _render_grade_section(db, grade, get_classes(db, grade.id))
        ...
        rows.append(
            {
                ...
                "角色": _role_label(r.role),
                "管辖范围": o.name if o else "?",
                "role_id": r.id,
            }
        )
    handler = _make_delete_role_handler(container, school_id, is_super)
...
def _refresh_roles(container, school_id, is_super):
    ...
    with container, get_db() as db:
        ...
        org_opts = _build_org_options(db, school, school_id)
        role_options = _available_role_options(is_super)

        _render_role_assignment_form(container, school_id, is_super, teacher_opts, org_opts, role_options)

        ui.label("当前授权").classes("text-md font-bold mt-4 mb-2")
        _render_authorization_list(db, container, school_id, is_super)
