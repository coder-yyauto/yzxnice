...
async def test_hierarchy():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            await page.screenshot(path=screenshot_dir / "06_hierarchy_test.png")
            print(f"  截图已保存: {screenshot_dir}/06_hierarchy_test.png")
            ...
            if school_count > 0:
                ...
                # 检查年级扩展面板
                grade_expansion = page.locator('.q-expansion-item:has-text("1年级")')
                grade_count = await grade_expansion.count()
                print(f"  找到 {grade_count} 个年级扩展面板")
                
                if grade_count > 0:
                    print("  ✅ 年级扩展面板显示正常")
                    
                    # 展开第一个年级
                    await grade_expansion.first().click()
                    await page.wait_for_timeout(1000)
                    ...
                    if "1年级" in page_content:
                        print("  ⚠️  页面包含'1年级'但未找到扩展面板")
                    else:
                        print("  ⚠️  页面不包含'1年级'")
